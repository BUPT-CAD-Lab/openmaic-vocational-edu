...
const GROUNDING_CONTEXT = '[Source: device-manual.pdf]\nInspect the device before calibration.';
...
<html>
  <body>
    ...
    expect(widgetPrompt.user).toContain('Retrieved Reference Material');
    expect(widgetPrompt.user).toContain(GROUNDING_CONTEXT);
    expect(widgetPrompt.user).not.toContain('{{languageDirective}}');
    expect(widgetPrompt.user).not.toContain('{{');

    const teacherActionPrompt = captured[1];
    expect(teacherActionPrompt.user).toContain('Retrieved Reference Material');
    expect(teacherActionPrompt.user).toContain(GROUNDING_CONTEXT);
  });
});
