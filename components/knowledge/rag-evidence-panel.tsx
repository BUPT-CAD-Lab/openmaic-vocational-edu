...
function EvidenceHit({ hit, compact }: { hit: RagHit; compact: boolean }) {
  const location = [
    hit.modality,
    hit.citation?.label,
    hit.citation?.pageNumber ? `第 ${hit.citation.pageNumber} 页` : undefined,
  ]
    .filter(Boolean)
    .join(' · ');
  ...
  return (
    <article className="rounded-md border border-border/70 bg-muted/25 p-2.5">
      ...
      {location && <div className="mb-1.5 text-[11px] text-muted-foreground">{location}</div>}
      {hit.imageSrc && (
        ...
        <img
          ...
          className={cn(
            'mb-2 max-h-44 rounded border border-border object-contain',
            compact && 'max-h-24',
          )}
        />
      )}
